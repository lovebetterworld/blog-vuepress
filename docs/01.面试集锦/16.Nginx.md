---
title: 16.Nginx
date: 2022-09-14 09:24:56
permalink: /interview/interview16/
categories: 
  - 面试题
tags: 
  - 面试题
...
由于 web server 的工作性质决定了每个 request 的大部份生命都是在网络传输中，实际上花费在 server 机器上的时间片不多。这是几个进程就解决[高并发](https://so.csdn.net/so/search?q=高并发&spm=1001.2101.3001.7020)的秘密所在。即：
...
[反向代理](https://so.csdn.net/so/search?q=反向代理&spm=1001.2101.3001.7020)（Reverse Proxy）方式，是指以代理服务器来接受 Internet上的连接请求，然后将请求，发给内部网络上的服务器并将从服务器上得到的结果返回给 Internet 上请求连接的客户端，此时代理服务器对外就表现为一个反向代理服务器。
...
Apache: 创建多个进程或线程，而每个进程或线程都会为其分配 cpu 和[内存](https://so.csdn.net/so/search?q=内存&spm=1001.2101.3001.7020)（线程要比进程小的多，所以 worker 支持比 perfork 高的并发），并发过大会榨干服务器资源。
...
其目的是，通过在现有的 Internet中 增加一层新的网络[架构](https://so.csdn.net/so/search?q=架构&spm=1001.2101.3001.7020)，将网站的内容发布到最接近用户的网络边缘，使用户可就近取得所需的内容，提高用户访问网站的速度。
...
![springboot + aop + Lua分布式限流的最佳实践](https://imgconvert.csdnimg.cn/aHR0cDovL3AzLnBzdGF0cC5jb20vbGFyZ2UvcGdjLWltYWdlLzJiNDJmYTEwNTA4NzQ5Mzc5MTZhNDFmNjZjYjlkYWJj?x-oss-process=image/format,png)
...
## 30 在nginx中，如何使用未定义的服务器名称来阻止处理请求？
...
![img](https://img-blog.csdnimg.cn/img_convert/53789ebbbed2c88d9f9643f177ced5c1.png)

现在我们开启了gzip进行压缩后的文件的大小，可以看到如下所示：

![img](https://img-blog.csdnimg.cn/img_convert/8620feba4fbe5595a4113f95c187685f.png)

并且我们查看响应头会看到gzip这样的压缩，如下所示

![img](https://img-blog.csdnimg.cn/img_convert/2b99a256b2811527930ce3168898a741.png)
...
## 35 ngx_http_upstream_module的作用是什么?

ngx_http_upstream_module用于定义可通过fastcgi传递、proxy传递、uwsgi传递、memcached传递和scgi传递指令来引用的服务器组。

## 36 生产中如何设置worker进程的数量呢？
